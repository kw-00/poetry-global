...
namespace PoetryGlobal.Features.Poems
{
    ...
    ) : IPoemOrchestration
    {
        ...
        private readonly IExternalPoetryAPIService _poetryDbService = poetryDbService;
        ...
        private readonly int _databaseSearchBatchSize 
            = configuration.GetFromConfigOrThrow<int>("Features:Poems:Search:DatabaseBatchSize");
        private readonly int _poetryDbSearchBatchSize
            = configuration.GetFromConfigOrThrow<int>("Features:Poems:Search:ExternalPoetryAPIBatchSize");

        private readonly int _preferredMinBatchSize
            = configuration.GetFromConfigOrThrow<int>("Features:Poems:Search:PreferredMinBatchSize");
        ...
        public async Task<(int PageCount, List<PersistedPoemMetadata> Page)> SearchAsync(SearchQueryDTO query, int pageNumber)
        {
            // Local helper function for caching search results and getting a page to return
            (int PageCount, List<PersistedPoemMetadata> Page) CacheResultsAndGetPage(List<PersistedPoemMetadata> metadata)
            {
                var pages = new SearchPages(metadata, _pageSize);
                _poemSearchCache.Set(query, pages);
                var page = pages.GetPage(pageNumber) ?? throw new NotFoundException("Page number out of bounds.");
            ...
            {
                var page = cachedPages.GetPage(pageNumber) ?? throw new NotFoundException("Page number out of bounds.");
                return (cachedPages.PageCount, [.. page]);
            }

            // If database returns enough poem metadata records, use them and return
            var metadataFromDatabase = await _poemMetadataRepository.SearchAsync(query, _databaseSearchBatchSize);
            ...
            var metadataFromExternalAPI = await _poetryDbService.GetPoemMetadataAsync(query, _poetryDbSearchBatchSize);
            ...
            // and let Total mean "total number of poems found in external API and database", deduplicated.
            // If N / Total < threshold, ignore external API results and use what was found in database.
            // If N / Total >= threshold, retrieve full poem texts from external API and save them to database,
            // then use both records from external API and from database.
            var potentialSuccessfulSaveRatio = totalPoemCount > 0 ? newPoemCount / totalPoemCount : 0;
            if (potentialSuccessfulSaveRatio >= _minSuccessfulSaveRatio)
            {
                var metadataAndVersions = await _poetryDbService.GetPoemsAsync(query, _poetryDbSearchBatchSize);
                ...
                var versionsToUpsert = persistedMetadata.Zip(versions, (m, v) => (Metadata: m, Version: v)).Select(mNv => new PoemVersion
                {
            ...
            // If external API returns too few results that were not in database, use database search results and return
            else
            {
                return CacheResultsAndGetPage(metadataFromDatabase);
            }
        ...
        public async Task<PoemDTO> GetPoemAsync(int poemMetadataId, int languageId)
        {
            var poemMetadata = await _poemMetadataRepository.GetAsync(poemMetadataId)
                ?? throw new NotFoundException($"Poem metadata ID of {poemMetadataId} was not found in database.");
            ...
            if (originalVersion is null)
            {
                ...
                throw new NotFoundException(
                    ...
                    + " has been removed, as poem metadata should not exist without corresponding original poem version."
                );
            }
